...
variable "database_transaction_log_retention_days" {
  description = "Database transaction log retention in days"
  type        = number
  default     = 7
  validation {
    condition     = var.database_transaction_log_retention_days >= 1 && var.database_transaction_log_retention_days <= 30
    error_message = "Database transaction log retention must be between 1 and 30 days."
  }
}
...
variable "database_default_disk_size_gb" {
  description = "Default disk size for database instances"
  type        = number
  default     = 10
  validation {
    condition     = var.database_default_disk_size_gb >= 10 && var.database_default_disk_size_gb <= 65536
    error_message = "Database disk size must be between 10 and 65536 GB."
  }
}

variable "database_autoresize_limit_gb" {
  description = "Maximum disk size for database autoresize"
  type        = number
  default     = 100
  validation {
    condition     = var.database_autoresize_limit_gb >= 10 && var.database_autoresize_limit_gb <= 65536
    error_message = "Database autoresize limit must be between 10 and 65536 GB."
  }
}

variable "database_query_string_length" {
  description = "Maximum query string length for database insights"
  type        = number
  default     = 1024
  validation {
    condition     = var.database_query_string_length >= 256 && var.database_query_string_length <= 2048
    error_message = "Query string length must be between 256 and 2048 characters."
  }
}